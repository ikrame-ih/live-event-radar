...
Each event follows: parse → ring `appendEvent` → trim at 10,000 → derive snapshots / ETA / tally → worker receives a **sample**, not the full FIFO.

## How the project grew

The first version was a single `/dashboard` route with a 4-panel grid and a Canvas 2D heatmap. Zone names were generic. Fine for a spike, not for recruiters.
...
Related: [Engineering decisions — design trade-offs and rationale](/engineering-decisions) · [Technical decisions](/technical-decisions) · [Current state](/current-state) · [Pipeline](/pipeline) · [Benchmarks](/benchmarks)
